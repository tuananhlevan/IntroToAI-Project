...
from helper import AleaUnc, TotalUnc


def visualize_prediction(model, X_train, y_train):
...
def visualize_uncertainty(model, X_train, y_train):
    plt.figure(figsize=(8, 6))
    plt.grid(True, linestyle='--', alpha=0.7)
    ...
    x_loc = torch.linspace(-25, 25, 100).unsqueeze(1)
    alea = AleaUnc(model=model, x=x_loc, num_sample=100)
    total = TotalUnc(model=model, x=x_loc, num_samples=100)
